...
Tracking work trough different sessions.
...
All files must be nested under `.ai/memory-bank/` within the project repository.
Files build upon each other in a clear hierarchy:

```mermaid
flowchart TD
    IL[Initial Load Prompt] --> AC[activeContext.md]
    AC --> P[progress.md]
    AC --> TF[tasks/ folder]
```
